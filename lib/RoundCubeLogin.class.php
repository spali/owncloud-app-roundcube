...
/**
 * Use a PHP script to perform a login to the Roundcube webmail
 *
 * @author Philipp Heckel
 * @author Martin Reinhardt <contact@martinreinhardt-online.de>
 * @author David Jaedke
 * @link   http://blog.philippheckel.com/2008/05/16/roundcube-login-via-php-script/
 * @version 20150128
 * @license WTFPL
 *
 * REQUIREMENTS
 ...
 *
 * INSTALLATION
 *   - Install Roundcube on your server so that it can be accessed via the browser
 ...
 *
 *         Returns: TRUE if the login succeeds, FALSE if the user/pass-combination is wrong
...
{
    /**
     * DNS name of the Roundcube server
     *
     ...
     *
     * Can be set via the second argument in the constructor.
     ...
     * @throws OC_Mail_NetworkingException
     */
    private function sendRequest($path, $postData = false)
    {
        ...
        $context = stream_context_create(array('http' => array(
            ...
            'header' => $header,
            'content' => $postData)));
        $responseObj = $this->openUrlConnection($url, $context);
        if (!$responseObj) {
            ...
            $response = $responseObj->getContent();
            ...
            $responseHdr = $responseObj->getHeader();
            $this->authHeaders = array();
     ...
     *
     * @param string $pURL
     * @param resource $pContext resource context
     * @return object response
     */
    function openUrlConnection($pURL, $pContext)
    {
        $response = false;
        $this->addDebug("openUrlConnection", "Opening URL (url:" . $pURL . ").");
        try {
            $this->rcUrlResource = fopen($pURL, 'rb', false, $pContext);
            if ($http_response_header && $this->rcUrlResource) {
                $response = new Response($http_response_header, stream_get_contents($this->rcUrlResource));
            } else {
    ...
    /**
     * Send authentication headers previously acquired
     */
